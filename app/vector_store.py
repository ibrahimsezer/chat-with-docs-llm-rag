...
def add_to_vector_store_faiss(chunks: list[str], doc_id: str):
    global vectorstore
...
def search_faiss(question: str, doc_id: str, top_k: int = 3):
    global vectorstore
    if vectorstore is None:
        print("[FAISS] Vectorstore boş, arama yapılamaz.")
        return []
    docs = vectorstore.similarity_search(question, k=top_k*3)  # Fazla döndürüp filtrele
    filtered_docs = [doc.page_content for doc in docs if doc.metadata.get("doc_id") == doc_id]
